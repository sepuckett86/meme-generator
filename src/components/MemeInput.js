...
import styles from './MemeInput.css';
import PropTypes from 'prop-types';

function MemeInput({ handleChange, handleUpload, handleClickUpdateImage, meme, exportMeme }) {
  return (
    <>
      <h3>Upload an image or a url</h3>
      <label>
        URL: 
        <input name="url" className={styles.textInput} type="text" onChange={handleChange} value={meme.url}/>
      ...
      <label>
        Header:
        <input name="header" className={styles.textInput} value={meme.header} onChange={handleChange}/>
        <input name="headerColor" type="color" value={meme.headerColor} onChange={handleChange}></input>
      </label>
      <label>
        Footer:
        <input name="footer" className={styles.textInput} value={meme.footer} onChange={handleChange}/>
...
MemeInput.propTypes = {
  ...
  exportMeme: PropTypes.func.isRequired
};
